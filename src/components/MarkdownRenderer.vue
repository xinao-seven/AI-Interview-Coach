...
<style>
/* ─── 基础样式 ──────────────────────────────── */
.markdown-renderer { font-size: 14px; line-height: 1.75; color: #303133; word-break: break-word; }
.markdown-renderer h1 { font-size: 1.5em; margin: 0.8em 0 0.4em; }
.markdown-renderer h2 { font-size: 1.25em; margin: 0.7em 0 0.3em; padding-bottom: 0.3em; border-bottom: 1px solid #ebeef5; }
.markdown-renderer h3 { font-size: 1.1em; margin: 0.6em 0 0.2em; }
.markdown-renderer p { margin: 0.4em 0; }
.markdown-renderer ul, .markdown-renderer ol { padding-left: 1.5em; margin: 0.4em 0; }
.markdown-renderer li { margin: 0.2em 0; }

.markdown-renderer code:not(pre code) {
  background: #f0f2f5; padding: 2px 6px; border-radius: 3px;
  font-size: 0.9em; color: #e6a23c;
  font-family: 'Consolas', 'Monaco', 'Courier New', monospace;
}

/* ─── 代码块（Atom One Dark 主题）───────────── */
.markdown-renderer .code-block-wrapper {
  position: relative; margin: 0.6em 0; border-radius: 8px; overflow: hidden;
}
.markdown-renderer .code-block-wrapper .code-lang {
  position: absolute; top: 0; left: 12px;
  font-size: 11px; color: #abb2bf; background: #3a3f4b;
  padding: 1px 10px; border-radius: 0 0 4px 4px; z-index: 1;
}
.markdown-renderer .code-block-wrapper .copy-btn {
  position: absolute; top: 6px; right: 10px;
  display: flex; align-items: center; gap: 4px;
  padding: 4px 10px; font-size: 12px;
  color: #abb2bf; background: #3a3f4b;
  border: 1px solid #4b5263; border-radius: 4px;
  cursor: pointer; opacity: 0; transition: opacity 0.2s; z-index: 1;
}
.markdown-renderer .code-block-wrapper .copy-btn:hover { background: #4b5263; color: #d7dae0; }
.markdown-renderer .code-block-wrapper:hover .copy-btn { opacity: 1; }
.markdown-renderer pre {
  background: #282c34; border-radius: 0; padding: 0; margin: 0; overflow-x: auto;
}
.markdown-renderer pre code {
  display: block; padding: 14px 16px; font-size: 13px; line-height: 1.7;
...
.markdown-renderer .thinking-fold {
  margin: 0.6em 0; border: 1px solid #e8d5b0;
  border-radius: 8px; overflow: hidden; background: #fefaf0;
}
...
.markdown-renderer .thinking-fold > summary::marker {
  display: none; content: '';  /* 标准 */
}
.markdown-renderer .thinking-fold > summary:hover { background: #fdf3dc; }

/* 自定义三角图标：折叠 ▸ / 展开 ▾ */
.markdown-renderer .thinking-fold-icon::before {
  content: '▸'; font-size: 12px; color: #b88230;
...
.markdown-renderer .thinking-fold-label {
  font-size: 13px; font-weight: 600; color: #b88230; white-space: nowrap;
}
.markdown-renderer .thinking-fold-summary {
  font-size: 12px; color: #c0a87a; overflow: hidden;
  text-overflow: ellipsis; white-space: nowrap; flex: 1;
}
.markdown-renderer .thinking-fold-body {
  padding: 10px 14px 14px; border-top: 1px solid #e8d5b0;
  font-size: 13px; color: #8b7355; line-height: 1.7; background: #fffcf5;
}

/* ─── 表格 / 引用 / 链接 ────────────────────── */
.markdown-renderer table { border-collapse: collapse; width: 100%; margin: 0.6em 0; }
.markdown-renderer th, .markdown-renderer td { border: 1px solid #dfe2e5; padding: 8px 12px; text-align: left; }
.markdown-renderer th { background: #f6f8fa; font-weight: 600; }
.markdown-renderer blockquote {
  border-left: 3px solid #409eff; padding: 4px 12px; margin: 0.4em 0;
  color: #606266; background: #ecf5ff; border-radius: 0 4px 4px 0;
}
.markdown-renderer a { color: #409eff; text-decoration: none; }
.markdown-renderer a:hover { text-decoration: underline; }
.markdown-renderer hr { border: none; border-top: 1px solid #ebeef5; margin: 0.6em 0; }

.variant-chat.markdown-renderer h2 { font-size: 1.15em; border-bottom-color: #faecd8; }
.variant-chat.markdown-renderer pre { background: #1e1e1e; }
.variant-chat.markdown-renderer pre code { color: #d4d4d4; }
.variant-report.markdown-renderer { font-size: 15px; }
</style>
