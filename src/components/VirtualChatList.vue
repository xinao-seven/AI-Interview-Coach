...
<template>
    <div ref="containerRef" class="virtual-chat-container" @scroll="handleScroll">
        ...
        <div class="virtual-chat-inner" :style="{ minHeight: totalHeight + 'px' }">
            <div v-for="vi in visibleItems" :key="vi.id" :ref="(el: any) => onItemRef(el, vi.id)"
                class="virtual-chat-item" :style="{ position: 'absolute', top: vi.top + 'px', left: 0, right: 0 }">
        ...
        <transition name="vb-fade">
            <div v-if="userScrollingUp && stickToBottom" class="scroll-bottom-btn" @click="scrollToBottom('smooth')">
                <span>↓ 回到底部</span>
