      ...
      <el-col :span="12">
        <el-card shadow="never" class="section-card">
          ...
          <div v-else class="history-list">
            <div
              v-for="(entry, i) in history.slice(0, 5)"
              :key="entry.sessionId"
              class="history-item"
            >
              ...
              <div class="history-info">
                ...
                <span class="history-mode">{{ modeMap[entry.mode] || entry.mode }} · {{ entry.questionCount }}题</span>
              </div>
              <span class="history-score" :style="{ color: scoreColor(entry.totalScore) }">
                {{ entry.totalScore }}分
              </span>
              <span class="history-time">{{ new Date(entry.finishedAt).toLocaleDateString('zh-CN') }}</span>
            </div>
          </div>
        </el-card>
      </el-col>
      ...
      <el-col :span="12">
        <el-card shadow="never" class="section-card">
          ...
          <div v-else class="weak-list">
            <div
              v-for="(entry, i) in topWeakPoints"
              :key="entry[0]"
              class="weak-item"
            >
              ...
              <el-progress
                :percentage="Math.round((entry[1] / topWeakPoints[0][1]) * 100)"
                :stroke-width="8"
                :color="i < 2 ? '#f56c6c' : i < 4 ? '#e6a23c' : '#909399'"
                :show-text="false"
                style="flex: 1"
              />
    ...
    <el-card shadow="never" class="section-card">
      ...
      <div class="recommend-list">
        ...
        <el-tag
          v-for="(entry, i) in topWeakPoints"
          :key="entry[0]"
          :type="i < 2 ? 'danger' : i < 4 ? 'warning' : 'info'"
          size="large"
          effect="plain"
          class="rec-tag"
        >
